...
unsigned int menuImg[N_GAMES][8] = {
	...
	{0,0,0x4000,0xC000,0x4000,0,0,0},
...
void spaceInvaders(){
	if (aux1 == 0){
		...
		pec[8] = 4;
		pec[9] = 16;		
		ligaLED(pec,pec[8],15);
		ligaLED(pec,pec[8]+1,16);
		ligaLED(pec,pec[8]-1,16);
		atribuirVetores(res,mapBreakout);
		contRot = 0;
	}
	
	limparVetores(imp,8);	
	unirVetores(bulletsVet,pec,imp);
	unirVetores(imp,res,imp);
	mandaImagem(imp);
	...
	if(B_ACTION == 0 && contBullet == 0){
		ligaLED(bulletsVet,pec[8],15);
		contBullet = 1;
	}	
	if(B_ACTION != 0 && contBullet != 0){
		contBullet = 0;
	...
	if(colisaoSuperiorRef(bulletsVet,res) == 1){
		//verificar a posição da colisão e destruir tanto o res quanto o bullet 
	}
...
void menu(){
	...
	if(gameMode > N_GAMES-1){gameMode = 0;}
	if(gameMode < 0){gameMode = N_GAMES-1;}
